...
class InterfaceRunner(HttpRunner):
    ...
    def __init__(self, interface_data: Dict):
        ...
        try:
            ...
            self.runtime_files = _resolve_runtime_files(project_id, self.interface_data.get('file_ids') or [])
    ...
    def _init_http_step(self):
        ...
        if self.interface_data.get('body'):
            ...
            if body is not None:
                ...
                step_obj = _apply_upload_files_to_step(step_obj, normalized_body, self.runtime_files)
                if normalized_body['type'] in {'form-data', 'x-www-form-urlencoded', 'binary'}:
